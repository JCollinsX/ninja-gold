...
Class.Mixin(Screen, {
	...
	initialize: function() {
	    if (this.SpecialParams === undefined) this.SpecialParams = '';
	...
	updateEmitterParticlesPosition: function(time) {
		...
		this.each(this.emitters, function(emitter) {
			...
			if (!emitter.skipUpdate) this.each(emitter.children, function(particle) {
				...
				//particle.alpha *= 0.98;
				particle.rotation += particle.accelerationRotation;
			}, this);
	...
	emitParticleWave: function(emitter) {
		...
		for (let i=0; (i < emitter.waveCount) && (!emitter.paused) && (emitter.particlesLimit > emitter.particlesCount); i++) {
			...
			var moviespeed = 0.3 //_.random(0.1, 0.5);
			var startIdx = 0;//_.random(0, emitter.images.length/2);
			sprite = this.buildChild(emitter, { type: 'movie-clip', frames: emitter.images, speed: moviespeed, loop: false, scale: 0.3 }, true);
			sprite.gotoAndPlay(startIdx);
			...
			if (emitter.particleAcceleration) {
			    sprite.accelerationX = this.getParticleValue(emitter.particleAcceleration[0]) || 0 + _.random(-100, 100);
			    sprite.accelerationY = this.getParticleValue(emitter.particleAcceleration[1]) || 0 + _.random(0, 200);
			}
			sprite.accelerationRotation = _.random(-50, 50) / 1000;
